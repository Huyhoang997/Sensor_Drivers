...
static TFT_Handler_t *temp_tft_handler = NULL;
...
TFT_Status_Typedef TFT_SetPos(TFT_Handler_t *Instance, uint8_t row_start, uint8_t row_end, uint8_t col_start, uint8_t col_end)
{
  ...
  status = TFT_SendData(Instance, 0x00, 1);
  if(status != TFT_OK) {
    return TFT_SPI_ERR;
  }

  status = TFT_SendData(Instance, row_start, 1);
  if(status != TFT_OK) {
    return TFT_SPI_ERR;
  }
  ...
  status = TFT_SendData(Instance, row_end, 1);
  if(status != TFT_OK) {
    return TFT_SPI_ERR;
  }
  ...
  status = TFT_SendData(Instance, col_start, 1);
  if(status != TFT_OK) {
    return TFT_SPI_ERR;
  }
  ...
  status = TFT_SendData(Instance, col_end, 1);
  if(status != TFT_OK) {
    return TFT_SPI_ERR;
  }
...
TFT_Status_Typedef TFT_Init(TFT_Handler_t *Instance)
{
  ...
  checkStatus = TFT_SendData(Instance, FRAME_MODE_1, 1);
  if(checkStatus != TFT_OK)
  {
      return TFT_ERR;
  }
  ...
  checkStatus = TFT_SendData(Instance, FRAME_MODE_3, 1);
  if(checkStatus != TFT_OK)
  {
      return TFT_ERR;
  }
  ...
  checkStatus = TFT_SendData(Instance, VCOM_PARA_1, 1);
  if(checkStatus != TFT_OK)
  {
      return TFT_ERR;
  }
  ...
  checkStatus = TFT_SendData(Instance, MADCTL_BGR, 1);
  if(checkStatus != TFT_OK)
  {
      return TFT_ERR;
  }
...
#define TOTAL_CHUNKS   (40960 / CHUNK_SIZE_DMA)
